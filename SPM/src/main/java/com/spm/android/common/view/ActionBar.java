...
public class ActionBar extends LinearLayout {
	...
	public ActionBar(Context context, AttributeSet attrs) {
		...
		actionsViewgroup = (LinearLayout)this.findViewById(R.id.actionsViewgroup);
		
		// logo.setOnClickListener(new LaunchOnClickListener());
		
		TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.actionBarStyleable);
		String title = typedArray.getString(R.styleable.actionBarStyleable_title1);
		if (title != null) {
	 ...
	 * @param clickListener
	 */
	public void addImageViewAction(int imageResource, OnClickListener clickListener) {
		ImageView imageAction = new ImageView(this.getContext());
		imageAction.setImageDrawable(this.getResources().getDrawable(imageResource));
	 ...
	 * @param clickListener
	 */
	public void addTextViewAction(int textResource, OnClickListener clickListener) {
		LayoutInflater layoutInflater = (LayoutInflater)getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		Button textAction = (Button)layoutInflater.inflate(R.layout.action_bar_text_button, this, false);
