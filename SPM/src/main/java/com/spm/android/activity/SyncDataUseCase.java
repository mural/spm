...
public class SyncDataUseCase extends DefaultAbstractUseCase {
	...
	@Inject
	public SyncDataUseCase(APIService apiService, OrderRepository orderRepository, ClientRepository clientRepository,
			VisitRepository visitRepository) {
		super(apiService);
	...
	protected void doExecute() {
		...
		if ((works == null) || works.isEmpty()) {
			throw CommonErrorCode.NOT_SELECTED_ITEMS_ERROR.newLocalBusinessException();
		}
		...
		for (Work work : works) {
			if (work instanceof Order) {
				if (!((Order)work).getProducts().isEmpty()) {
					...
					sync = getApiService().sync((Order)work, clientRepository.get(((Order)work).getClientId()));
			...
			if (work instanceof Visit) {
				if ((((Visit)work).getComment() != null) && (((Visit)work).getComment().length() > 0)) {
		...
		if (empty > 0) {
			result = result.concat("\nLos Pedidos sin productos o Notas sin comentarios no se sincronizaron");
		}
	}
}