...
public class LoginUseCase extends DefaultAbstractUseCase {
	...
	protected void doExecute() {
		...
		TelephonyManager tMgr = (TelephonyManager)AndroidApplication.get().getSystemService(Context.TELEPHONY_SERVICE);
		String phoneNumber = tMgr.getLine1Number();
		// if ((user != null) && (user.getFirstName().length() > 0) && (phoneNumber != null)
		// && phoneNumber.equals(user.getPhoneNumber()) && user.checkValidDate()) {
		if ((user != null) && (user.getFirstName().length() > 0) /* && user.checkValidDate() */) { // for rapid access
		
			user = new User(user.getId(), user.getFirstName(), user.getClients(), user.getUpdateDate(),
					user.getOrderNumber(), user.getUsersUpdateDate());
			Application.APPLICATION_PROVIDER.get().attach(user);
		} else {
			throw CommonErrorCode.USER_NOT_FOUND_ERROR.newLocalBusinessException();
		}
	}
