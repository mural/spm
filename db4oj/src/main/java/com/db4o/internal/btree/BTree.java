...
public class BTree extends LocalPersistentBase implements TransactionParticipant, BTreeStructureListener {
    ...
    public BTree(Transaction trans, BTreeConfiguration config, Indexable4<IdSlotMapping> keyHandler) {
	...
	private void freeAllNodeIds(LocalTransaction systemTrans, final Iterator4<Integer> allNodeIDs) {
		TransactionalIdSystem idSystem = idSystem(systemTrans);
        while(allNodeIDs.moveNext()){
            int id = (allNodeIDs.current()).intValue();
            idSystem.notifySlotDeleted(id, slotChangeFactory());
        }
	}

	public Iterator4<Integer> allNodeIds(Transaction systemTrans) {
		final Collection4<Integer> allNodeIDs = new Collection4<Integer>();
	...
	public void evictedFromCache(BTreeNode node){
		//TODO_evictedFromCache = Tree.add(_evictedFromCache, new TreeIntObject(node.getID(), node));
	}
